# Table of contents

* [Decoupled Drupal with Gatsby](README.md)

## Fundamentals

* [React](fundamentals/react.md)
* [Gatsby](fundamentals/gatsby.md)
* [GraphQL](fundamentals/graphql.md)
* [JAMStack](fundamentals/jamstack.md)
* [JSX](fundamentals/jsx.md)
* [Webpack](fundamentals/webpack.md)

## Getting Started

* [Build your first Gatsby site](getting-started/build-your-first-gatsby-site.md)
* [Project structure](getting-started/project-structure.md)
* [Component structure](getting-started/component-structure.md)

## Components

* [About Components](components/components.md)
* [Eyebrow](components/eyebrow.md)
* [Button](components/button.md)
* [Working with CSS](components/working-with-css.md)
* [Movie Card](components/movie-card.md)

## Gatsby Core Components

* [Gatsby Link](gatsby-core-components/gatsby-link.md)
* [Gatsby Image](gatsby-core-components/gatsby-image.md)

## Data and GraphQL

* [Data sources](data-and-graphql/data-sources.md)
* [Connecting a data source](data-and-graphql/connecting.md)
* [Using the GraphQL playground](data-and-graphql/playground.md)

## CreatePages

* [createPages Function](createpages/create-pages-function.md)
* [Create a template](createpages/create-a-template.md)

## Resources

* [Gatsby and React Resources](resources/gatsby-and-react-resources.md)
* [Other resources](resources/other-resources.md)

